  ...
  const generateMenuItems = useCallback(
    (menuList) => {
      ...
      return menuList.filter(filterMenuItem).map((item) => {
        const isOpen = item.children?.some((child) => path.indexOf(child.path) === 0);
  ...
  return (
    <div className="sidebar-menu-container">
      <Scrollbars autoHide autoHideTimeout={1000} autoHideDuration={200}>
        <Menu mode="inline" theme="dark" onSelect={handleMenuSelect} selectedKeys={[location.pathname]} openKeys={openKeys} onOpenChange={onOpenChange} items={menuItems} />
      </Scrollbars>
    </div>
  );
};
